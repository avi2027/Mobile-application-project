  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (context, groupSnapshot) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text('Bachelor Mess Meals'),
            actions: [
              IconButton(
                icon: const Icon(Icons.bar_chart),
                onPressed: () {
                  context.push('/group/${widget.groupId}/meal-settlement');
                },
              ),
              if (!isGroupClosed)
                IconButton(
...
class _MealCountsList extends StatelessWidget {
  final String groupId;
  final MealStatistics stats;

  const _MealCountsList({
    required this.groupId,
    required this.stats,
  });
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, UserModel>>(
      ...
      builder: (context, snapshot) {
        ...
        final mealCountsList = stats.mealCounts.entries.toList()
          ..sort((a, b) => b.value.compareTo(a.value)); // Sort by meal count descending

        if (mealCountsList.isEmpty) {
          return const Padding(
            padding: EdgeInsets.all(16),
            child: Text(
        ...
        return Column(
          children: mealCountsList.map((entry) {
